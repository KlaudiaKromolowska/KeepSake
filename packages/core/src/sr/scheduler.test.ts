...
describe("onSessionStartOutcome — pre-mastery growth walk", () => {
  it("grows the gap ×1.5 on each success, capping at 14 days", () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      expect(next.gapDays).toBeLessThanOrEqual(config.gapCapDays);
      state = frozen(next);
    }
...
describe("onSessionStartOutcome — unclear", () => {
  it("is a strict no-op in between mode: schedule fields unchanged, no reopen", () => {
    // The return type always adds `reopenWithinSession`, so a bare object-identity check
    // (`toBe`) against the bare `ScheduleState` input is impossible — this asserts the
    // schedule itself (mode/gapDays/boosterStep/nextDueAt) is byte-for-byte unchanged instead.
    const state = frozen(afterCeilingHandoff(0, config));
    const result = onSessionStartOutcome(state, "unclear", DAY_MS, config);
    expect(result).toEqual({ ...state, reopenWithinSession: false });
  });
});
...
describe("onBoosterOutcome — success walk", () => {
  it("advances the cadence 7 -> 14 -> 30 -> 90, then clamps at 90", () => {
    ...
    expect(s1.boosterStep).toBe(1);
    expect(s1.nextDueAt).toBe(cadenceDays(1) * DAY_MS);
    expect(s1.reopenWithinSession).toBe(false);
    state = frozen(s1);

    const s2 = onBoosterOutcome(state, "recall", 0, config);
    expect(s2.boosterStep).toBe(2);
    state = frozen(s2);

    const s3 = onBoosterOutcome(state, "recall", 0, config);
    expect(s3.boosterStep).toBe(3);
    expect(s3.nextDueAt).toBe(cadenceDays(3) * DAY_MS);
    state = frozen(s3);

    // Already at the last step — success clamps, repeating the 90d cadence.
    const s4 = onBoosterOutcome(state, "recall", 0, config);
    expect(s4.boosterStep).toBe(3);
    expect(s4.nextDueAt).toBe(cadenceDays(3) * DAY_MS);
  });
});
...
describe("onBoosterOutcome — miss drops one cadence step", () => {
  it("drops from step 2 to step 1 (+14d) and requests reopen", () => {
    ...
    expect(result.boosterStep).toBe(1);
    expect(result.nextDueAt).toBe(cadenceDays(1) * DAY_MS);
  ...
  it("stays at step 0 on a miss (never below step 0)", () => {
    ...
    const result = onBoosterOutcome(state, "miss", 0, config);
    expect(result.boosterStep).toBe(0);
    expect(result.nextDueAt).toBe(cadenceDays(0) * DAY_MS);
    expect(result.reopenWithinSession).toBe(true);
  });
});

describe("onBoosterOutcome — unclear", () => {
  it("is a strict no-op: schedule fields unchanged, no reopen", () => {
    const state = frozen(afterMastery(0, config));
