...
//
// A second memory ("Kraków") that reached mastery and now runs its booster loop, so the demo shows
// the multi-target queue: two settled targets running check-ins in parallel while a third (below)
// is still acquiring. Four distinct-day sessions — one teach + three start-probe recalls — take the
// start streak to masteryStreak (3), which the reducer itself ends as `mastered`. Every value still
// traces to real @keepsake/core output. Mastered on day −8 with a 7-day first booster cadence → the
// next check-in falls a day ago, so it reads as "due" on camera.
...
function driveAcquiringArc(): {
  ...
  if (candidacy.status !== "passed")
    throw new Error(`acquiring candidacy: expected passed, got ${candidacy.status}`);

  cursor += 5_000; // candidacy done, begin teaching
  let a1 = startSession(initialProgress, { at: cursor, timeZone: PATIENT_TZ }, config);
  li = 0;
  ...
  return {
    candidacyTrials: candidacy.trials,
    sessions: [
      { startedAt: sessionStart, endedAt: a1End, trials: a1.trials },
      { startedAt: day(1), endedAt: a2End, trials: a2.trials },
    ],
    finalProgress: a2.progress,
  };
}
...
async function seed(): Promise<void> {
  ...
  // Two more targets so the demo shows the V2 multi-target queue: a second mastered booster
  // ("Kraków") running in parallel, and a third still-acquiring memory ("Ewa") that holds the
  // single acquisition slot and is the one the kiosk opens a session on (see driveAcquiringArc).
  await seedMasteredTarget(admin, patientId);
  await seedAcquiringTarget(admin, patientId);

  await printSummary(admin, caregiverId, patientId, targetId);
}

/**
 * Insert the second, already-mastered target ("Kraków") and its four-session booster-loop history
 * so the demo exercises the V2 multi-target queue (mastered boosters running alongside the
 * acquiring target). Status "mastered" is startable (PRACTICABLE_STATUSES) so its boosters can run.
 */
...
async function seedAcquiringTarget(admin: SupabaseClient, patientId: string): Promise<void> {
  const { data: target, error: targetErr } = await admin
    ...
    .single();
  if (targetErr || !target)
    throw new Error(`acquiring target insert failed: ${targetErr?.message}`);
  const targetId = target.id as string;

  const arc = driveAcquiringArc();
  ...
  for (const [i, session] of arc.sessions.entries()) {
    const { data: sessionRow, error: sessionErr } = await admin
      ...
      .single();
    if (sessionErr || !sessionRow)
      throw new Error(`acquiring session insert failed: ${sessionErr?.message}`);
    const trials = i === 0 ? [...arc.candidacyTrials, ...session.trials] : session.trials;
    const rows = trials.map((tr, idx) => trialRow(sessionRow.id, targetId, tr, trialCount + idx));
    trialCount += rows.length;
    const { error: trialsErr } = await admin.from("trials").insert(rows);
    if (trialsErr) throw new Error(`acquiring trials insert failed: ${trialsErr.message}`);
  }
  ...
  if (stateErr) throw new Error(`acquiring target_state insert failed: ${stateErr.message}`);
}
